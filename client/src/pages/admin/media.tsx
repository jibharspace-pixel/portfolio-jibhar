...
import { Trash2, Loader2, ImagePlus, Eye, Image, Video, BarChart3, Globe, Cog, Brain } from "lucide-react";
...
const CAT_COLORS: Record<string, string> = { data: "bg-blue-50 text-blue-700 border-blue-200", web: "bg-primary/8 text-primary border-primary/20", automation: "bg-amber-50 text-amber-700 border-amber-200", ai: "bg-purple-50 text-purple-700 border-purple-200" };
const CAT_LABELS: Record<string, string> = { data: "Data & BI", web: "Web", automation: "Auto.", ai: "IA" };

function UploadBtn({ projectId, accept, label, icon: Icon, password, onDone }: {
  projectId: string; accept: string; label: string; icon: LucideIcon; password: string; onDone: () => void;
}) {
  ...
  const handleFile = async (file: File) => {
    ...
    await fetch(`/api/admin/projects/${projectId}/upload`, { method: "POST", headers: { "x-admin-password": password }, body: fd });
    onDone();
    setUploading(false);
...
export function MediaSection({ password }: { password: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        <div className="grid md:grid-cols-2 gap-4">{[1,2,3,4].map(i => <div key={i} className="h-40 rounded-2xl bg-muted/40 animate-pulse" />)}</div>
      ) : (
